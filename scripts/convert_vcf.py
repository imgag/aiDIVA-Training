...
def calculate_homAF(row):
    if (row["gnomAD_AN"] != "") and (row["gnomAD_Hom"] != ""):
        if (float(row["gnomAD_Hom"]) > 0.0) and (float(row["gnomAD_AN"]) > 0.0):
...
if __name__=="__main__":
    ...
    parser.add_argument("--indel", dest="indel", action="store_true", required=False, help="Flag to indicate whether the file to convert consists of indel variants or not.\n")
    ...
    vcf_as_dataframe = convert_vcf_to_pandas_dataframe(args.in_data, is_indel, threads)
